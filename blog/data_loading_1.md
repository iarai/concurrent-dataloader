...
- As the experiment uses 4 workers, it’s evident that batches are acquired in parallel as red lines seem to be in parallel (it’s easier to notice it on the first image). However, a deeper dive into the code shows that the implementation of the `fetcher` collects batch items sequentially. 
