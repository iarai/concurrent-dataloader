...
class MPActionPlayer(ActionPlayer):
    ...
    def run(self, action_name: str, action: Callable, repeat_action: int = 20):
        ...
        action_name = action_name + "_pooled_run_" + action.__name__ + "_" + "pid:" + str(os.getpid())
        logging.info(f"Repeating... {repeat_action}")
        for _ in range(repeat_action):
            self.stopwatch.record(action_name)
            action()
            self.stopwatch.record(action_name)
        results = self.stopwatch.get_results(action_name, False)["total"]
        logging.info("Done repeating... %s: %s", repeat_action, results)
        return results
    ...
    def benchmark(self, action_name: str, action: Callable, repeat: int, verbose: bool = False,) -> None:
        ...
        # TODO according
        with Pool(
            self.pool_size, initializer=partial(initialize_logging, loglevel=logging.getLogger().getEffectiveLevel())
        ) as pool:
