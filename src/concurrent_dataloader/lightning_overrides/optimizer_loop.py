...
class OptimizerLoop(Loop[_OUTPUTS_TYPE]):
    ...
    def _run_optimization(
        ...
        optimizer_timeline_id = time.time() + batch_idx 
        logging.getLogger("timeline").debug(json.dumps({
            "item": "fwd_bck_step",
            "id": optimizer_timeline_id,
        ...
        loss_update_id = time.time() + batch_idx
        logging.getLogger("timeline").debug(json.dumps({
            "item": "update_loss",
            "id": loss_update_id,
            "start_time": time.time()
    ...
    @stopwatch(trace_name="(7)-_training_step", trace_level=7)
